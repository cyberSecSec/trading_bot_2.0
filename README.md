...
### Старый способ (setup.py)

> **Устарело!** Теперь рекомендуется использовать Poetry и pyproject.toml.
...
4. **Обработчик сообщений о ликвидациях**
   - Класс `LiquidationMessageHandler` для обработки и парсинга сообщений
   - Выделение данных о ликвидированных позициях
   - Логирование крупных ликвидаций для мониторинга рынка

5. **Анализатор ликвидаций**
   - Класс `LiquidationAnalyzer` для выявления значимых событий в потоке ликвидаций
   - Обнаружение крупных ликвидаций (выше порогового значения объема)
   - Обнаружение каскадных ликвидаций (серии ликвидаций в короткий промежуток времени)
   - Сбор и анализ статистики по ликвидациям

6. **Готовый пример использования**
   - Демонстрационный скрипт для подписки на ликвидации нескольких инструментов
   - Реализация обработки и вывода информации о ликвидациях
   - Статистика и анализ происходящих на рынке событий

7. **Модульные тесты**
   - Тесты для проверки корректной работы подписки на ликвидации
   - Тесты для обработчика сообщений о ликвидациях
   - Тесты для анализатора ликвидаций с проверкой обнаружения паттернов

Сервис для данных о ликвидациях предоставляет ценную информацию для анализа рыночных условий, выявления потенциальных точек разворота и понимания распределения рисков на рынке.
...
### Сервис для данных о ликвидациях

Реализован сервис для получения данных о ликвидациях через WebSocket. Сервис включает в себя:

1. **WebSocket подписка на поток ликвидаций**
   - Метод `subscribe_to_liquidations` в классе `BybitMarketDataStreamProvider`
   - Обработка WebSocket-сообщений о ликвидациях в реальном времени
...
4. **Готовый пример использования**
   - Демонстрационный скрипт для подписки на ликвидации нескольких инструментов
   - Реализация обработки и вывода информации о ликвидациях
   - Статистика и анализ происходящих на рынке событий
